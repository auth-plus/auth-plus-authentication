...
} from '../../core/driven/creating_mfa.driven'
import {
  FindingMFA,
  FindingMFAErrors,
  FindingMFAErrorsTypes,
} from '../../core/driven/finding_mfa.driven'
import { UpdatingUser } from '../../core/driven/updating_user.driven'
import { ValidatingMFA } from '../../core/driven/validating_mfa.driven'
...
export class MFARepository implements CreatingMFA, FindingMFA, ValidatingMFA {
    ...
    logger.debug(
      {
        ...
        action: 'creatingStrategyForUser',
        userId: user.id,
        strategy,
      },
      'Database query: insert MFA strategy'
    )
  ...
  async findMfaListByUserId(
    ...
    logger.debug(
      { table: this.tableName, action: 'findMfaListByUserId', userId },
      'Database query: select active MFA strategies'
    )
    const tuples = await this.database<MFARow>(this.tableName)
      .select('*')
  ...
  async findMFAByUserIdAndStrategy(
    ...
    logger.debug(
      {
        table: this.tableName,
        action: 'findMFAByUserIdAndStrategy',
        userId,
        strategy,
      },
      'Database query: select MFA strategy by user and strategy'
    )
    const tuples = await this.database<MFARow>(this.tableName)
      .select('*')
      .where('user_id', userId)
  ...
  async validate(mfaId: string): Promise<boolean> {
    logger.debug(
      { table: this.tableName, action: 'validate', mfaId },
      'Database query: update MFA enable status'
    )
