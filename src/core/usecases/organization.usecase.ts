  ...
  implements CreateOrganization, AddUserToOrganization, UpdateOrganization
{
  ...
  async create(name: string, parentId: string | null): Promise<string> {
    logger.info(
      { event: 'org.create.started', name, parentId },
      'Organization creation initiated'
    )
    try {
      const orgId = await this.creatingOrganization.create(name, parentId)
      logger.info(
        { event: 'org.create.success', organizationId: orgId, name, parentId },
      ...
      if (err.message === CreatingOrganizationErrorsTypes.PARENT_NOT_EXIST) {
        logger.warn(
          {
            event: 'org.create.failed',
            name,
            parentId,
            reason: 'parent_not_exist',
          },
          'Organization creation failed: parent organization does not exist'
        )
      ...
      logger.error(
        { event: 'org.create.error', name, parentId, error: err.message },
        'Organization creation failed: internal or dependency error'
      )
      throw new CreateOrganizationErrors(
  ...
  async addUser(organizationId: string, userId: string): Promise<string> {
    logger.info(
      { event: 'org.user.add.started', organizationId, userId },
      'Adding user to organization initiated'
    )
    try {
      ...
      switch (err.message) {
        ...
        case FindingUserErrorsTypes.USER_NOT_FOUND:
          logger.warn(
            {
              event: 'org.user.add.failed',
              organizationId,
              userId,
              reason: 'not_found',
              error: err.message,
            },
            'Adding user to organization failed: entity not found'
          )
          ...
          logger.warn(
            {
              ...
              organizationId,
              userId,
              reason: 'duplicated_relationship',
            },
            'Adding user to organization failed: relationship already exists'
          )
          throw new AddUserToOrganizationErrors(
        ...
        default:
          logger.error(
            {
              event: 'org.user.add.error',
              organizationId,
              userId,
              error: err.message,
            },
            'Adding user to organization failed: internal or dependency error'
          )
          throw new AddUserToOrganizationErrors(
  ...
  async update(
    ...
    logger.info(
      { event: 'org.update.started', organizationId, name, parentId },
      'Organization update initiated'
    )
    try {
      ...
      logger.info(
        { event: 'org.update.success', organizationId, name, parentId },
      ...
      switch (err.message) {
        case FindingOrganizationErrorsTypes.ORGANIZATION_NOT_FOUND:
          logger.warn(
            { event: 'org.update.failed', organizationId, reason: 'not_found' },
        ...
        case UpdatingOrganizationErrorsTypes.CYCLIC_RELATIONSHIP:
          logger.warn(
            {
              event: 'org.update.failed',
              organizationId,
              reason: 'cyclic_relationship',
            },
            'Organization update failed: cyclic relationship detected'
          )
          throw new UpdateOrganizationErrors(
        ...
        default:
          logger.error(
            { event: 'org.update.error', organizationId, error: err.message },
            'Organization update failed: internal or dependency error'
          )
          throw new UpdateOrganizationErrors(
