...
export default class MFA implements CreateMFA, ValidateMFA, ListMFA {
  ...
  async create(userId: string, strategy: Strategy): Promise<string> {
    logger.info(
      { event: 'auth.mfa.setup.started', userId, strategy },
      'MFA setup initiated'
    )
    try {
      ...
      logger.info(
        { event: 'auth.mfa.setup.success', userId, strategy },
        'MFA setup completed and verification challenge sent'
      )
      return mfa.secret ?? ''
    } catch (error) {
      ...
      switch (err.message) {
        ...
        case CreatingMFAErrorType.MFA_ALREADY_EXIST:
          logger.warn(
            ...
            'MFA setup failed: strategy already exists'
          )
        ...
        case CreatingMFAErrorType.MFA_INFO_NOT_EXIST:
          logger.warn(
            ...
            'MFA setup failed: missing registration info'
          )
          throw new CreateMFAErrors(CreateMFAErrorsTypes.INFO_NOT_EXIST)
        default:
          logger.error(
            {
              event: 'auth.mfa.setup.error',
              userId,
              strategy,
              error: err.message,
            },
            'MFA setup failed: dependency or internal error'
          )
  ...
  async validate(mfaId: string): Promise<boolean> {
    logger.info(
      { event: 'auth.mfa.validation.started', mfaId },
      'MFA validation initiated'
    )
    try {
      const result = await this.validatingMFA.validate(mfaId)
      logger.info(
        { event: 'auth.mfa.validation.success', mfaId, success: result },
      ...
      logger.error(
        {
          event: 'auth.mfa.validation.error',
          mfaId,
  ...
  async list(userId: string): Promise<Strategy[]> {
    logger.info(
      { event: 'auth.mfa.list.started', userId },
      'Listing user MFA strategies'
    )
    try {
      const user = await this.findingUser.findById(userId)
      ...
      logger.info(
        { event: 'auth.mfa.list.success', userId, strategies },
        'User MFA strategies listed successfully'
      )
      return strategies
    } catch (error) {
      const err = error as Error
      if (err.message === FindingUserErrorsTypes.USER_NOT_FOUND) {
        logger.warn(
          { event: 'auth.mfa.list.failed', userId, reason: 'user_not_found' },
          'Listing user MFA strategies failed: user not found'
        )
        throw new ListMFAErrors(ListMFAErrorsTypes.USER_NOT_FOUND)
      }
      logger.error(
        { event: 'auth.mfa.list.error', userId, error: err.message },
        'Listing user MFA strategies failed'
      )
