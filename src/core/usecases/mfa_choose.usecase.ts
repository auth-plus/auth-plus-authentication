...
export default class MFAChoose implements ChooseMFA {
  ...
  async choose(hash: string, strategy: Strategy): Promise<string> {
    logger.info(
      { event: 'auth.mfa.choose.started', strategy },
      'MFA strategy choice initiated'
    )
    try {
      ...
      logger.info(
        { event: 'auth.mfa.choose.success', userId: resp.userId, strategy },
        'MFA strategy chosen and verification code sent'
      )
      return newHash
    } catch (error) {
      ...
      switch (err.message) {
        ...
        case SendingMfaCodeErrorsTypes.USER_EMAIL_NOT_FOUND:
          logger.warn(
            {
              event: 'auth.mfa.choose.failed',
              strategy,
              reason: 'not_found',
              error: err.message,
            },
            'MFA strategy choice failed: target not found'
          )
          ...
          logger.warn(
            {
              event: 'auth.mfa.choose.failed',
              strategy,
              reason: 'strategy_not_listed',
            },
            'MFA strategy choice failed: strategy not listed'
          )
          throw new ChooseMFAErrors(ChooseMFAErrorsTypes.STRATEGY_NOT_LISTED)
          ...
          logger.warn(
            {
              event: 'auth.mfa.choose.failed',
              strategy,
              reason: 'sending_code_error',
              error: err.message,
            },
            'MFA strategy choice failed: phone code transmission error'
          )
          throw new ChooseMFAErrors(ChooseMFAErrorsTypes.SENDING_CODE_ERROR)
        default:
          logger.error(
            { event: 'auth.mfa.choose.error', strategy, error: err.message },
            'MFA strategy choice failed: dependency or internal error'
          )
