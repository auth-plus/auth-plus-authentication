  ...
  implements ForgetPassword, RecoverPassword
{
  ...
  async forget(email: string): Promise<void> {
    logger.info({ event: 'auth.password.forget.started', email }, 'Password reset request initiated')
    try {
      const hash = await this.creatingResetPassword.create(email)
      await this.sendingResetEmail.sendEmail(email, hash)
      logger.info({ event: 'auth.password.forget.success', email }, 'Password reset token generated and email sent')
    } catch (error) {
      logger.error({ event: 'auth.password.forget.error', email, error: (error as Error).message }, 'Password reset request failed: dependency or internal error')
  ...
  async recover(newPassword: string, hash: string): Promise<void> {
    logger.info({ event: 'auth.password.recover.started' }, 'Password recovery execution initiated')
    let resolvedEmail = ''
    try {
      ...
      logger.info({ event: 'auth.password.recover.success', email: resolvedEmail }, 'Password updated successfully')
    } catch (error) {
      logger.error({ event: 'auth.password.recover.error', email: resolvedEmail || undefined, error: (error as Error).message }, 'Password recovery failed: dependency or internal error')
