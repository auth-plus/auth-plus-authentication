...
describe('mfa_choose repository', () => {
  const mockHash = casual.uuid,
    userId = casual.uuid,
    strategyList: Strategy[] = [Strategy.EMAIL]
  ...
  it('should succeed when creating a mfa hash', async () => {
    ...
    const uuidService: UuidService = instance(mockUuidService),
      mFAChooseRepository = new MFAChooseRepository(redis, uuidService),
      result = await mFAChooseRepository.create(userId, strategyList)
